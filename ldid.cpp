	...
	_foreach (file, files) try {
		...
		if (flag_S) {
			...
			std::vector<CodesignAllocation> allocations; {
				_foreach (mach_header, source.GetMachHeaders()) {
					...
					special = std::max(special, (uint32_t)CSSLOT_CODEDIRECTORY);
					...
					special = std::max(special, (uint32_t)CSSLOT_REQUIREMENTS);
					alloc += sizeof(struct BlobIndex);
					alloc += 0xc;

					if (xmld != NULL) {
						special = std::max(special, (uint32_t)CSSLOT_ENTITLEMENTS);
						alloc += sizeof(struct BlobIndex);
					...
					alloc = Align(alloc + (do_sha1?(special + normal) * CS_SHA1_LEN:0) + (do_sha2?(special + normal) * CS_SHA256_LEN:0), 16);
		...
		_foreach (mach_header, fat_header.GetMachHeaders()) {
			...
			if (flag_s) {
				...
				for (size_t index(0); index != Swap(super->count); ++index)
					if (Swap(super->index[index].type) == CSSLOT_CODEDIRECTORY) {
						...
						if (directory->hashType == CS_HASHTYPE_SHA1) {
							uint8_t (*hashes)[CS_SHA1_LEN] = reinterpret_cast<uint8_t (*)[CS_SHA1_LEN]>(blob + begin + Swap(directory->hashOffset));
						...
						} else if (directory->hashType == CS_HASHTYPE_SHA256) {
							uint8_t (*hashes)[CS_SHA256_LEN] = reinterpret_cast<uint8_t (*)[CS_SHA256_LEN]>(blob + begin + Swap(directory->hashOffset));
							uint32_t pages = Swap(directory->nCodeSlots);
							...
							fprintf(stderr, "unknown directory->hashType: 0x%02x", directory->hashType);
							_assert(false);
						}
					}
			}
			...
			if (flag_S) {
				...
				uint8_t (*sha256hashes)[CS_SHA256_LEN] = NULL;
				uint8_t (*sha1hashes)[CS_SHA1_LEN] = NULL;
				...
				if (do_sha1) {
					// SHA1
					super->index[idx].type = Swap(CSSLOT_CODEDIRECTORY);
					super->index[idx].offset = Swap(offset);

					uint32_t begin = offset;
					struct CodeDirectory *sha1directory = reinterpret_cast<struct CodeDirectory *>(blob + begin);
					...
					sha1directory->nSpecialSlots = Swap(special);

					sha1hashes = reinterpret_cast<uint8_t (*)[CS_SHA1_LEN]>(blob + offset);
					memset(sha1hashes, 0, sizeof(*sha1hashes) * special);

					offset += sizeof(*sha1hashes) * special;
					sha1hashes += special;

					sha1directory->nCodeSlots = Swap(pages);

					if (pages != 1)
						for (size_t i = 0; i != pages - 1; ++i)
							sha1(sha1hashes[i], top + 0x1000 * i, 0x1000);
					if (pages != 0)
						sha1(sha1hashes[pages - 1], top + 0x1000 * (pages - 1), ((data - 1) % 0x1000) + 1);

					sha1directory->hashOffset = Swap(offset - begin);
					offset += sizeof(*sha1hashes) * pages;
					sha1directory->blob.length = Swap(offset - begin);
					idx++;
				...
				if (do_sha2) {
					...
					uint32_t begin = offset;
					struct CodeDirectory *sha256directory = reinterpret_cast<struct CodeDirectory *>(blob + offset);
					offset += sizeof(struct CodeDirectory);
					...
					sha256directory->nSpecialSlots = Swap(special);

					sha256hashes = reinterpret_cast<uint8_t (*)[CS_SHA256_LEN]>(blob + offset);
					memset(sha256hashes, 0, sizeof(*sha256hashes) * special);

					offset += sizeof(*sha256hashes) * special;
					sha256hashes += special;

					sha256directory->nCodeSlots = Swap(pages);

					if (pages != 1)
						for (size_t i = 0; i != pages - 1; ++i)
							sha2(sha256hashes[i], top + 0x1000 * i, 0x1000);
					if (pages != 0)
						sha2(sha256hashes[pages - 1], top + 0x1000 * (pages - 1), ((data - 1) % 0x1000) + 1);

					sha256directory->hashOffset = Swap(offset - begin);
					offset += sizeof(*sha256hashes) * pages;
					sha256directory->blob.length = Swap(offset - begin);
					idx++;
				}

				super->index[idx].type = Swap(CSSLOT_REQUIREMENTS);
				super->index[idx++].offset = Swap(offset);
				...
				if (xmld != NULL) {
					super->index[idx].type = Swap(CSSLOT_ENTITLEMENTS);
					super->index[idx++].offset = Swap(offset);
					...
					entitlements->magic = Swap(CSMAGIC_EMBEDDED_ENTITLEMENTS);
				...
				for (size_t index(0); index != count; ++index) {
					...
					if (type != 0 && type <= special) {
						...
						if (do_sha1 && sha1hashes != NULL)
							sha1((uint8_t *) (sha1hashes - type), (uint8_t *) local, Swap(local->length));
						if (do_sha2 && sha256hashes != NULL)
							sha2((uint8_t *) (sha256hashes - type), (uint8_t *) local, Swap(local->length));
					}
				}
